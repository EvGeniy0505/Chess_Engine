...
std::tuple<int, int, int, int> AIMoveGenerator::generateBestMove(Board &board) {
    ...
    for (const auto &move : possibleMoves) {
        ...
        tempBoard.make_move({fromX, fromY}, {toX, toY});
    ...
    return possibleMoves.empty() ? std::make_tuple(0, 0, 0, 0)
                                 : possibleMoves[0];
}

int AIMoveGenerator::evaluatePosition(Board &board, int depth, bool maximizing,
                                      int alpha, int beta) {
    if (depth == 0 || board.is_checkmate(ai_color)) {
    ...
    if (maximizing) {
        ...
        for (const auto &move : moves) {
            ...
            int eval =
                evaluatePosition(tempBoard, depth - 1, false, alpha, beta);
        ...
        Color opponent =
            (ai_color == Color::WHITE) ? Color::BLACK : Color::WHITE;
        ...
        for (const auto &move : moves) {
            ...
            int eval =
                evaluatePosition(tempBoard, depth - 1, true, alpha, beta);
...
int AIMoveGenerator::calculateBoardScore(const Board &board) {
    ...
    for (int y = 0; y < 8; ++y) {
        for (int x = 0; x < 8; ++x) {
            ...
            switch (
                piece
                    .get_type()) { // Changed from piece.type to piece.getType()
                case PieceType::PAWN:
                    pieceValue = 100;
                    break;
            ...
            score += (piece.get_color() == ai_color)
                         ? pieceValue
                         : -pieceValue; // Changed from piece.color to
                                        // piece.getColor()
        }
    }

    // Простая позиционная оценка (можно расширить)
    if (board.is_check(ai_color)) {
        score -= 50;
    }
    if (board.is_check(ai_color == Color::WHITE ? Color::BLACK
                                                : Color::WHITE)) {
        score += 50;
    }
...
AIMoveGenerator::generateAllMoves(const Board &board, Color color) {
    ...
    for (int y = 0; y < 8; ++y) {
        for (int x = 0; x < 8; ++x) {
            if (board.get_piece({x, y}).get_color() ==
                color) { // Changed from piece.color to piece.getColor()
                auto pieceMoves = board.get_legal_moves({x, y});
