...
std::optional<Move> OpeningBook::parseMove(const std::string& moveStr) {
    if (moveStr.size() < 4) return std::nullopt;

    auto fileToIndex = [](char file) -> int {
        if (file >= 'a' && file <= 'h') return file - 'a';
        return -1;
    };
...
std::optional<Move> OpeningBook::getOpeningMove(const Board &board, Color color) const {
    ...
    if (it == book_.end() || it->second.empty()) return std::nullopt;
    ...
    std::vector<std::pair<Move, int>> topMoves(moves.begin(), moves.begin() + limit);
    std::sort(topMoves.begin(), topMoves.end(), [](auto& a, auto& b) {
...
std::string OpeningBook::removeMoveCounters(const std::string& fen) {
    ...
    if (lastSpace == std::string::npos) return fen;
    size_t secondLast = fen.rfind(' ', lastSpace - 1);
    if (secondLast == std::string::npos) return fen;
